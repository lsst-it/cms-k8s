...
kubectl create secret generic etcd-certs -n yagan-prom --from-file=ca.crt=ca.crt --from-file=client.crt=etcd-client.crt --from-file=client.key=etcd-client.key --dry-run=client -o yaml > etcd-certs.yaml
...
helm install yagan-prom -n yagan-prom prometheus-community/kube-prometheus-stack -f values.yaml --version=43.1.3