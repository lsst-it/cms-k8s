...
kubectl create ns manke-prom
helm repo add prometheus-community https://prometheus-community.github.io/helm-charts
helm repo update
ssh ${USER}@manke01.ls.lsst.org 'sudo cat /etc/kubernetes/ssl/kube-ca.pem' > ca.crt
ssh ${USER}@manke01.ls.lsst.org 'sudo cat /etc/kubernetes/ssl/kube-ca-key.pem' > ca.key
...
kubectl create secret generic etcd-certs -n manke-prom --from-file=ca.crt=ca.crt --from-file=client.crt=etcd-client.crt --from-file=client.key=etcd-client.key --dry-run=client -o yaml > etcd-certs.yaml
...
helm install manke-prom -n manke-prom prometheus-community/kube-prometheus-stack -f values.yaml --version=43.1.3