...
helm repo update
ssh ${USER}@gaw01.ls.lsst.org 'sudo cat /etc/kubernetes/ssl/kube-ca.pem' > ca.crt
ssh ${USER}@gaw01.ls.lsst.org 'sudo cat /etc/kubernetes/ssl/kube-ca-key.pem' > ca.key
...
helm install gaw-prom -n gaw-prom prometheus-community/kube-prometheus-stack -f values.yaml --version=43.1.3