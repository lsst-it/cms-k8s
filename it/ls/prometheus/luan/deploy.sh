...
helm repo update
ssh ${USER}@luan01.ls.lsst.org 'sudo cat /etc/kubernetes/ssl/kube-ca.pem' > ca.crt
ssh ${USER}@luan01.ls.lsst.org 'sudo cat /etc/kubernetes/ssl/kube-ca-key.pem' > ca.key
...
kubectl create secret generic etcd-certs -n luan-prom --from-file=ca.crt=ca.crt --from-file=client.crt=etcd-client.crt --from-file=client.key=etcd-client.key --dry-run=client -o yaml > etcd-certs.yaml
...
helm install luan-prom -n luan-prom prometheus-community/kube-prometheus-stack -f values.yaml --version=43.1.3